...
class Chatbot:
    ...
    def _process_completion(
        ...
        #     "<|im_end|>"
        # )
        if completion["choices"][0]["text"].endswith(
            "<|im_end|>"
        ):
            completion["choices"][0]["text"] = completion["choices"][0]["text"][:-len("<|im_end|>")]
...
def one_turn(prompt, api_key=None):
    print(api_key)
    chatbot = Chatbot(api_key=api_key)
    response = chatbot.ask(prompt, temperature=0.5)
...
if __name__ == "__main__":
    one_turn("Explain yourself to me")
    # main()
